                  ...
                  char *filename)
{
    ...
    for (MAT_PTR_TYPE i = 0; i < nnz_mtx_report; i++)
    {
        ...
        csrRowIdx_tmp[i] = idxj;
        csrColIdx_tmp[i] = idxi;
        csrVal_tmp[i] = fval;
    }
    ...
    nnz_tmp = csrRowPtr_counter[m_tmp];
    int *csrColIdx_alias = (int *)malloc(nnz_tmp*2 * sizeof(int));
    MAT_VAL_TYPE *csrVal_alias    = (MAT_VAL_TYPE *)malloc(nnz_tmp*2 * sizeof(MAT_VAL_TYPE));
